...
def newFrame(id):
    ...
    labelTitle = Label(frame1,text=se.getTitlef(),font=("arial italic", 18),bg='#a855ff',fg='white' )
    ...
    labelImg = Label(frame1, image=img)
    labelImg.image = img
    labelImg.place(x=10, y=80, relwidth=0.5, relheight=0.4)

    publisher = Label(frame1, text='Publisher : '+se.getPublisherf(), font=("arial italic", 14), bg='#a855ff', fg='white' )
    publisher.place(x=400,y=200)

    source = Button(frame1, text='Source', font=("arial italic", 12), bg='#a855ff',fg='white',command=partial(openUrl,se.getSourceUrlf()))
    source.place(x=400, y=250)

    recipeTilte = Label(frame1,text='Recipe :',bg='#a855ff',fg='#66FF33', font=("arial italic", 20,'bold'))
    ...
    recipeList = Listbox(recipeFrame, width=400, yscrollcommand = scrollbar2.set,font=("arial italic", 14))
...
def findBtn():
    ...
    for d in data:
        ...
        frame3=Frame(myFrame,width=600,height=100,bg='blue')
        frame3.pack()
        ...
        moreInfo = Button(frame3,text='More Info',fg='white',bg='#3498DB',command=partial(newFrame,d['recipe_id']))
        moreInfo.place(relwidth=0.2,relx=0.7,rely=0.45)

        Frame(myFrame, height=5, width=600, pady=10, bg='white').pack()

root = Tk()
canvas = Canvas(root,height=1080,width=1920,bg='#263D42')
canvas.pack()

frame1 = LabelFrame(root,bg='#a855ff')
frame2 = LabelFrame(root,bg='#edaaff')
frame1.place(relheight=1.0,relwidth=0.495,x=0,y=0)
frame2.place(relheight=1.0,relwidth=0.495,relx=0.505,y=0)

searchBox = Text(frame2,height=1,bg='#E5E7E9')
searchBox.config(font =("Courier", 14))
searchBox.place(relwidth=0.6,relx=0.2,rely=0.01)
searchBox.insert(END,'Search...')

submit = Button(frame2,text='Find',fg='white',bg='#3498DB',command=findBtn)
submit.place(relwidth=0.2,relx=0.4,rely=0.05)

frame3 = LabelFrame(frame2,bg='#6872fd')
frame3.place(relheight=0.8,relwidth=0.9,relx=0.05,rely=0.1)

#Below is a procedure to add a scroll to a frame
...
for i in range(20):
      Frame(myFrame, height=100, width=600, pady=10, bg='white').pack()
      Frame(myFrame, height=5, width=600, pady=10, bg='white').pack()
root.mainloop()